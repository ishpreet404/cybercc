      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14">
          ...
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-sm bg-terminal-green/10 border border-terminal-green flex items-center justify-center">
              <Shield className="w-5 h-5 text-terminal-green animate-pulse" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="font-mono font-extrabold text-base tracking-wider text-terminal-green text-shadow-terminal">
                  CYBER CHAUKIDAAR
                </span>
                <span className="text-[10px] font-mono px-1.5 py-0.2 bg-terminal-green/20 text-terminal-green border border-terminal-green/40">
                  v1.2.0
                </span>
              </div>
              <div className="text-[10px] font-mono text-terminal-muted tracking-widest uppercase">
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {item.badge > 0 && (
                    <span className="ml-1 px-1.5 py-0.2 text-[10px] bg-terminal-red text-white font-bold rounded-sm animate-pulse">
                      {item.badge}
                    </span>
                  )}
                </button>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-1.5 text-[11px] font-mono">
              <span className={`w-2 h-2 rounded-full ${wsConnected ? 'bg-terminal-green animate-pulse' : 'bg-terminal-red'}`} />
            ...
            {/* Time */}
            <div className="hidden lg:block text-xs font-mono text-terminal-muted px-2 py-1 border border-terminal-border bg-black/40">
              {timeStr}
            </div>
          </div>
        </div>
        ...
        <div className="md:hidden flex items-center justify-between py-2 border-t border-terminal-border/50 overflow-x-auto space-x-1">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => setActiveTab(item.id)}
              className={`px-2 py-1 text-[11px] font-mono uppercase whitespace-nowrap border ${
                activeTab === item.id ? 'border-terminal-green text-terminal-green bg-terminal-green/10' : 'border-transparent text-gray-400'
              }`}
            >
              {item.label}
            </button>
