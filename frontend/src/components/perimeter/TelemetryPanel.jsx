...
import { Battery, BatteryCharging, BatteryWarning, Wifi, Radio, Clock, Cpu, Eye, Activity } from 'lucide-react';
import Card from '../Card';

export const TelemetryPanel = ({ nodes = [], selectedNodeId, onSelectNode }) => {
  return (
    <Card title="▸ DISTRIBUTED NODE TELEMETRY & HEALTH">
        ...
        {nodes.map((node) => {
          ...
          return (
            <div
              ...
              className={`p-3 bg-terminal-surface border cursor-pointer transition-all ${
                isSelected
                  ? 'border-terminal-green shadow-[0_0_10px_rgba(0,255,102,0.2)] bg-terminal-surface/90'
            ...
            >
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-1.5">
                  <span className="font-mono text-xs font-bold text-gray-100">{node.nodeId}</span>
                  <span className="text-[10px] font-mono text-terminal-muted truncate max-w-[90px]">
                    {node.name.replace('Perimeter Node ', '')}
                  </span>
                </div>
                <span className={`px-1.5 py-0.2 text-[9px] font-mono font-bold uppercase border ${statusBadgeColor}`}>
                  {node.status}
                </span>
              </div>

              {/* Battery & Voltage Gauge */}
              <div className="mb-2 p-2 bg-terminal-black border border-terminal-border">
                <div className="flex items-center justify-between text-[11px] font-mono mb-1">
                  <span className="text-terminal-muted flex items-center gap-1">
                    {isLowBat ? (
                      <BatteryWarning className="w-3.5 h-3.5 text-terminal-amber" />
                    ) : (
                      <Battery className="w-3.5 h-3.5 text-terminal-green" />
                    )}
                    BATTERY:
                  </span>
              ...
              <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-[10px] font-mono text-gray-300">
                <div className="flex items-center justify-between border-b border-terminal-border/40 py-0.5">
                  <span className="text-terminal-muted flex items-center gap-1">
                    <Radio className="w-2.5 h-2.5" /> RSSI:
                  </span>
                  <span className="font-bold">{node.signal?.rssi || -60} dBm</span>
                </div>

                <div className="flex items-center justify-between border-b border-terminal-border/40 py-0.5">
                  <span className="text-terminal-muted flex items-center gap-1">
                    <Wifi className="w-2.5 h-2.5" /> COMMS:
                  </span>
                ...
                <div className="flex items-center justify-between border-b border-terminal-border/40 py-0.5">
                  <span className="text-terminal-muted flex items-center gap-1">
                    <Eye className="w-2.5 h-2.5" /> RADAR:
                  </span>
                ...
                <div className="flex items-center justify-between border-b border-terminal-border/40 py-0.5">
                  <span className="text-terminal-muted flex items-center gap-1">
                    <Activity className="w-2.5 h-2.5" /> SEISMIC:
                  </span>
              ...
              <div className="mt-2 pt-1 border-t border-terminal-border/40 flex items-center justify-between text-[9px] font-mono text-terminal-muted">
